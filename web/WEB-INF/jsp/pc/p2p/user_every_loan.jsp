    ...
    <body>
        ...
        <table class="table table-striped">
            ...
            <c:forEach items="${out['result']}" var="res">
                <tr><td>${res['number']+1}</td><td>¥${res['repay_interest']}</td><td>¥${res['repay_capital']}</td><td>¥${res['repay_sum']}</td><td>¥${res['surplus_loan']}</td> <td><c:if test="${res['status']=='0'}"><input type="button" onclick="hk(${res['every_load_id']},${res['load_id']},${res['number']+1});" value="还款"></c:if></td></tr>
            </c:forEach>
        </table>
